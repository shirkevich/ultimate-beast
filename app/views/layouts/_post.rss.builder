xm.item do
  ...
  xm.title "{title} posted by {user} @ {date}"[key, h(post.respond_to?(:topic_title) ? post.topic_title : post.topic.title), h(post.user.beauty_name), post.created_at.rfc822]
  ...
  xm.author "#{post.user.beauty_name}"
